...
from npcpy.llm_funcs import get_llm_response as npcpy_get_llm_response
...
def calculate_accuracy_metrics(results: List[Dict]):
    ...
    if not results:
        return {'accuracy': 0.0, 'total_scenarios': 0, 'correct_count': 0}
    ...
    return {'accuracy': accuracy, 'total_scenarios': total_scenarios, 'correct_count': correct_count}
...
class ConferenceSimulator:
    ...
    def _generate_hidden_coeffs(self, seed: int) -> Dict[str, float]:
        ...
        return { "energy_decay_base": rng.uniform(0.1, 0.2), "introvert_energy_multiplier": rng.uniform(1.5, 2.5),
    ...
    def _calculate_meeting_success(self, person_a: Person, person_b: Person, time_slot: TimeSlot) -> Tuple[bool, float]:
        ...
        return success, float(success_prob)
...
Write a brief description of how this person appears and behaves right now."""
        ...
        return npcpy_get_llm_response(prompt, model="qwen3:1.7b", provider="ollama", temperature=self.temperature).get("response")
...
class MeetingPredictor:
    def __init__(self, model_path: str):
        ...
        base_model = AutoModelForCausalLM.from_pretrained("google/gemma-3-270m-it", trust_remote_code=True, torch_dtype=torch.float32, attn_implementation="eager")
    ...
    def predict(self, person_a_desc: str, person_b_desc: str, time_slot: str) -> dict:
        ...
        with torch.no_grad():
            outputs = self.model.generate(**inputs, max_new_tokens=24, do_sample=False, use_cache=True, pad_token_id=self.tokenizer.pad_token_id, eos_token_id=self.tokenizer.eos_token_id)
        text = self.tokenizer.decode(outputs[0, input_length:], skip_special_tokens=True).strip()
        match = re.search(r"\\{.*?\\}", text, flags=re.DOTALL)
        if match is None:
            raise ValueError(f"No JSON object found in SFT response: {text}")
        result = json.loads(match.group(0))
        if "reason" not in result or "probability" not in result:
            raise ValueError(f"SFT response missing required fields: {result}")
        result["reason"] = str(result["reason"]).strip()
        result["probability"] = float(result["probability"])
        if not 0.0 <= result["probability"] <= 1.0:
            raise ValueError(f"Probability outside [0,1]: {result['probability']}")
        return result
...
def safe_extract_json(response_text: str) -> dict:
    ...
    try:
        ...
        patterns = [ r'<start_of_turn>model\n(.*?)(?:<end_of_turn>|$)', r'model\n(.*?)(?:<end_of_turn>|$)',
...
def run_local_agent_evaluation( model: AutoModelForCausalLM, tokenizer: AutoTokenizer, test_scenarios: List[Dict],
    ...
    results = []
    def custom_llm_response(prompt, model_name=None, provider=None, format=None, **kwargs):
    ...
    for persona_idx, persona in enumerate(system_prompt_configurations):
        for scenario in test_scenarios:
            ...
            try:
                ...
                current_agent = NPC( name=persona["name"].lower(), primary_directive=persona["primary_directive"],
                    tools=TOOLS, )
...
Begin your analysis by calling a tool."""
                ...
                results.append({ 'scenario_id': scenario_id, 'persona': persona['name'], 'is_correct': is_correct,
                    'agent_outcome': agent_outcome, 'true_outcome': true_outcome, })
